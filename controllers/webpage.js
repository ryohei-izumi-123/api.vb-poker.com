...
class WebpagesController {
   ...
   * @memberof WebpagesController
   */
  validationMiddleware(method = '') {
    switch (method) {
      default:
        return Models.webpages.getValidators(method)
    }
  }
  ...
  async getAll(req, res, next) {
    try {
      ...
      const condition = {
        attributes: { exclude: ['content'] }, // content: LONGTEXTなので一覧では表示しないため除外（パフォーマンスのため）
        where: _.pickBy(where, _.identity),
        order: [[sort, order]]
      }

      const { count = 0 } = await Models.webpages.findAndCountAll(condition)
      ...
      /** @type {Array<ModelWebpages>} */
      const result = await Models.webpages.findAll(condition)
  ...
  async getOne(req, res, next) {
    try {
      ...
      /** @type {ModelWebpages} */
      const { dataValues } = await Models.webpages.findOne({
        where: _.pickBy(where, _.identity)
      })
  ...
  async create(req, res, next) {
    ...
    try {
      ...
      const { pageTypes, locale, content, status } = input
      ...
      /** @type {ModelWebpages} */
      const model = await Models.webpages.create(payload, options)
      const { id = undefined } = model

      await tx.commit()
      tx = null

      const { dataValues } = await Models.webpages.findByPk(id, {
        useMaster: true
      })
  ...
  async update(req, res, next) {
    ...
    try {
      ...
      const { id, pageTypes, locale, content, status } = input
      const userId = _.get(req, 'user.id')
      const updatedId = userId
      const model = await Models.webpages.findByPk(id, options)
  ...
  async destroy(req, res, next) {
    try {
      ...
      const { id } = input
      const model = await Models.webpages.findByPk(id)
      await model.destroy()
