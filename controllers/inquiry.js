...
class InquiriesController {
   ...
   * @memberof InquiriesController
   */
  validationMiddleware(method = '') {
    return Models.inquiries.getValidators(method)
  }
  ...
  async getAll(req, res, next) {
    try {
      ...
      const { count = 0 } = await Models.inquiries.findAndCountAll(condition)
      ...
      /** @type {Array<ModelInquiries>} */
      const result = await Models.inquiries.findAll(condition)
  ...
  async getOne(req, res, next) {
    try {
      ...
      /** @type {ModelInquiries} */
      const { dataValues } = await Models.inquiries.findOne({
        where: _.pickBy(where, _.identity)
      })
  ...
  async create(req, res, next) {
    ...
    try {
      ...
      /** @type {ModelInquiries} */
      const model = await Models.inquiries.create(payload, options)
      const { id = undefined } = model

      await tx.commit()
      tx = null

      const { dataValues } = await Models.inquiries.findByPk(id, {
        useMaster: true
      })
  ...
  async update(req, res, next) {
    ...
    try {
      ...
      const userId = _.get(req, 'user.id')
      const updatedId = userId
      const model = await Models.inquiries.findByPk(id, options)
  ...
  async destroy(req, res, next) {
    try {
      ...
      const { id } = input
      const model = await Models.inquiries.findByPk(id)
      await model.destroy()
