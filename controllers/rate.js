...
class RatesController {
  ...
  validationMiddleware(method = '') {
    return Models.rates.getValidators(method)
  }
  ...
  async getAll(req, res, next) {
    try {
      ...
      const { count = 0 } = await Models.rates.findAndCountAll(condition)
      ...
      /** @type {Array<ModelRates>} */
      const result = await Models.rates.findAll(condition)
  ...
  async getOne(req, res, next) {
    try {
      ...
      /** @type {ModelRates} */
      const { dataValues } = await Models.rates.findOne({
        where: _.pickBy(where, _.identity)
      })
  ...
  async create(req, res, next) {
    ...
    try {
      ...
      /** @type {ModelRates} */
      const model = await Models.rates.create(payload, options)
      const { id = undefined } = model

      await tx.commit()
      tx = null

      const { dataValues } = await Models.rates.findByPk(id, {
        useMaster: true
      })
  ...
  async update(req, res, next) {
    ...
    try {
      ...
      const userId = _.get(req, 'user.id')
      const updatedId = userId
      const model = await Models.rates.findByPk(id, options)
  ...
  async destroy(req, res, next) {
    try {
      ...
      const { id } = input
      const model = await Models.rates.findByPk(id)
      await model.destroy()
