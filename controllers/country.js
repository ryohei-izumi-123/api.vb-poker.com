...
class CountriesController {
  ...
  validationMiddleware(method = '') {
    return Models.countries.getValidators(method)
  }
  ...
  async getAll(req, res, next) {
    try {
      ...
      const { count = 0 } = await Models.countries.findAndCountAll(condition)
      ...
      /** @type {Array<ModelCountries>} */
      const result = await Models.countries.findAll(condition)
  ...
  async getOne(req, res, next) {
    try {
      ...
      /** @type {ModelCountries} */
      const { dataValues } = await Models.countries.findOne({
        where: _.pickBy(where, _.identity)
      })
